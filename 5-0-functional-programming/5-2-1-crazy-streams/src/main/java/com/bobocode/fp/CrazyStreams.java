...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(Collectors.partitioningBy(
                        account -> account.getSex() == Sex.MALE
                ));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .mapToInt(accounts -> accounts.getFirstName().length()
                + accounts.getLastName().length())
                        .sum();
                    }
    ...
    public Map<Long, Account> collectAccountsById() {
        return accounts.stream()
                .collect(Collectors.toMap(account -> account.getId(),
                        account -> account)
                );
    }
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream()
                ...
                .collect(Collectors.toMap(account -> account.getEmail(),
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream()
                .collect(Collectors.groupingBy(account -> account.getLastName(),
                        Collectors.mapping(account -> account.getFirstName(),
                                Collectors.toSet()
                        )
                ));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                ...
                .flatMap(firstName -> firstName.chars()
                .mapToObj(c -> (char) c))
                        .collect(Collectors.groupingBy(
                                character -> character,
                Collectors.counting()
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                .flatMap(account ->
        Stream.of(account.getFirstName(), account.getLastName())
                )
